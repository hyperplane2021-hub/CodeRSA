"""Core functions extracted from the GoRSA notebooks."""
import io
import json
import hashlib
import math
...
def write_json(obj: dict, path: str | Path) -> None:
    ...
    with open(tmp_path, "w", encoding="utf-8") as f:
        ...
        f.flush()
        if os.environ.get("GORSA_JSON_FSYNC", "1") not in {"0", "false", "False", "no", "NO"}:
            os.fsync(f.fileno())
...
def candidate_passes_mbpp(
    code: str,
    test_setup_code: str,
    test_list: Sequence[str],
    timeout_seconds: int = 8,
) -> Tuple[bool, str]:
    """Execute candidate code and MBPP asserts in a subprocess."""
    preamble = textwrap.dedent(
        """\
        import math
        import random
    ...
    if test_setup_code.strip():
        script.append(test_setup_code.strip())
    script.append(code.rstrip())
    script.extend(test_list)
    joined = "\n\n".join(script) + "\n"

    with tempfile.TemporaryDirectory() as td:
        file_path = Path(td) / "mbpp_exec.py"
        file_path.write_text(joined, encoding="utf-8")
        proc = None
        try:
...
def load_mbpp_test_split(limit: Optional[int] = None):
    """Load the MBPP+ test split used by the notebook reproduction."""
    try:
        ds = load_dataset("evalplus/mbppplus", split="test")
    except Exception as e:
        raise RuntimeError(f"Unable to load MBPP+ test split: {e}") from e
...
def normalize_mbpp_doc(doc: dict) -> dict:
    """Normalize an EvalPlus MBPP+ sample into the pipeline schema."""
    test_imports = doc.get("test_imports", []) or []
    test_setup_code = "\n".join(str(x) for x in test_imports).strip()
    public_tests = [str(x) for x in doc.get("test_list", [])]
    full_test_blob = str(doc.get("test", "") or "").strip()

    return {
        "task_id": int(doc["task_id"]),
        "text": str(doc.get("prompt", "")).strip(),
        "test_list": public_tests,
        "test_setup_code": test_setup_code,
        "challenge_test_list": [full_test_blob] if full_test_blob else [],
        "reference_code": str(doc.get("code", "") or "").strip(),
    }
...
def build_mbpp_generation_prompt(doc: dict) -> str:
    description = doc["text"].strip()
    test_example = doc["test_list"][0].strip() if doc.get("test_list") else ""
...
def initialize_task_files_mbpp(config: ReproConfig, dataset) -> None:
    ensure_dir(config.root_dir)
    write_json({"config": config.to_dict()}, Path(config.root_dir) / "run_config.json")
...
def align_mbpp_instruction_style(text: str) -> str:
    """Make generated behavior descriptions look like MBPP task instructions."""
    text = text.strip()
    if not text:
        return text
    ...
    if lower.startswith("return "):
        ...
        return f"Write a function that returns {body}"
    ...
    if lower.startswith(mbpp_prefixes):
        return text[0].upper() + text[1:]

    body = text[:-1] if text.endswith((".", "!", "?")) else text
    body = body[0].lower() + body[1:] if body else body
    return f"Write a function to {body}."
...
def postprocess_generated_instruction(text: str) -> str:
    ...
    text = align_mbpp_instruction_style(text)
    return text
